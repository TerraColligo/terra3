...
class SubmittedTest(models.Model):
    ...
    test_type_name = fields.Char('Test Type Name', related='test_type.name', store=True)
    ...
    order_name = fields.Char('Order Name', related='order.name', store=True, track_visibility='onchange')
    order_partner_name = fields.Char('Order Partner Name', related='order.partner_id.name', store=True, track_visibility='onchange')
    ...
    submitted_sample_create_date = fields.Datetime('Submitted Sample Create Date', related='submitted_sample.create_date', store=True, track_visibility='onchange')
    ...
    @api.depends('submitted_test_variables', 'submitted_test_variables.test_variable_type', 'submitted_test_variables.value')
    ...
    # When this test is modified, pass on the chatter message to the Order object
    def _message_log(self, *args, **kwargs):
        ...
        # Add more info about the variable to the chatter message (by default, field_desc is just "Value")
        submitted_sample = None
        submitted_test = None
        for item in self:
            submitted_test = item
            submitted_sample = item.submitted_sample

        newargs = dict(kwargs)
        if submitted_sample and submitted_test:
            if 'tracking_value_ids' in newargs:
                for tracking_value in newargs['tracking_value_ids']:
                    if len(tracking_value) >= 3:
                        attrs = tracking_value[2]
                        if 'field_desc' in attrs:
                            attrs['field_desc'] = '%s / %s %s' % (submitted_sample.name, submitted_test.test_type.name, attrs['field_desc'])
            logger.info('MESSAGE LOG NEWARGS %s' % (newargs))

        for item in self:
            if hasattr(item.order, '_message_log'):
                item.order._message_log(*args, **newargs)
