...
class SubmittedTestVariable(models.Model):
    ...
    # When this variable is modified, pass on the chatter message to the Order object
    def _message_log(self, *args, **kwargs):
        ...
        for item in self:
            submitted_test_variable = item
            submitted_sample = item.submitted_sample
            submitted_test = item.submitted_test

        newargs = dict(kwargs)
        if submitted_sample and submitted_test and submitted_test_variable:
                ...
                for tracking_value in newargs['tracking_value_ids']:
                    if len(tracking_value) >= 3:
                        ...
                        if 'field_desc' in attrs:
                            attrs['field_desc'] = '%s / %s / Variable %s %s' % (submitted_sample.name, submitted_test.test_type.name, submitted_test_variable.num, attrs['field_desc'])
