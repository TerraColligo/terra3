...
class SpreadsheetGenerateDemoOrders(object):
    ...
    def import_demo_orders(self: OdooModel, rows: SpreadsheetRows, partner_mapping: PartnerMapping) -> None:
        ...
        def save_order(current_order: CurrentOrder, row_num: int):
            ...
            logger.info('ORDER TEST PRODUCTS:')
            for key, value in current_order['test_products'].items():
                for key2, value2 in value['tests'].items():
                    logger.info('- %s: %s: %s' % (key, key2, value2))
            ...
            logger.info('CONFIGURING TEST VARIABLES FOR ALL ORDER LINES IN ORDER %s' % (order))
            ...
            for order_line in order.order_line:
                ...
                if not test_product_code:
                    ...
                    logger.info('  - PROCESSING ORDER LINE TESTS: %s' % (tests))
                    for terralab_submitted_test in order_line.terralab_submitted_tests:
                        test_variables = tests.get(terralab_submitted_test.test_type.default_code, None)
                        if not test_variables:
                            raise ValidationError('Test %s not found in product %s in order starting on row %s' % (terralab_submitted_test.test_type.default_code, test_product_code, current_order['row']))
